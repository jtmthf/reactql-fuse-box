...
export default function createNewStore(apolloClient: ApolloClient) {
  const store = createStore(
    ...
    !process.env.SERVER ? window.__STATE__ : {}, // initial state
    compose(
      applyMiddleware(apolloClient.middleware()),
      // Enable Redux Devtools on the browser, for easy state debugging
      // eslint-disable-next-line no-underscore-dangle
      (!process.env.SERVER && typeof window.__REDUX_DEVTOOLS_EXTENSION__ !== 'undefined') ? window.__REDUX_DEVTOOLS_EXTENSION__() : (f: any) => f,
    ),
  );
